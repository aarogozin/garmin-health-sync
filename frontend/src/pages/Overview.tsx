...
export function Overview() {
  ...
  return <>
    <PageHeader eyebrow={new Intl.DateTimeFormat('en', { weekday: 'long', month: 'long', day: 'numeric' }).format(new Date())} title="Today" description="Your current signals with a selectable, read-only context." actions={<><button className="button" onClick={() => setCustomizing(!customizing)}><Settings2 />Customize</button><div className="segmented" aria-label="Dashboard period">{([1, 7, 30] as const).map((days) => <button key={days} className={period === days ? 'active' : ''} onClick={() => setPeriod(days)}>{days === 1 ? 'Today' : `${days} days`}</button>)}</div><button className="button button--primary" disabled={busy} onClick={() => void start('/dashboard/refresh', { period_days: period })}><RefreshCw />Refresh dashboard</button></>} />
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}><SortableContext items={order} strategy={verticalListSortingStrategy}><section className="score-grid">{order.map((id) => <SortableScore key={id} item={cards[id]} id={id} editing={customizing} />)}</section></SortableContext></DndContext>
