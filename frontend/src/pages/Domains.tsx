...
export function DomainPage({ domain }: { domain: keyof typeof configs }) {
  ...
  return <><PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description} actions={<><SegmentedControl label="Period" value={period} onChange={setPeriod} options={[{ value: 1, label: 'Today' }, { value: 7, label: '7 days' }, { value: 30, label: '30 days' }]} /><button className="button" disabled={busy} onClick={() => void start('/dashboard/refresh', { period_days: period })}>Refresh snapshot</button></>} />{charts.length ? <section className="chart-grid">{charts.map((chart) => <article className="panel" key={chart.id}><div className="panel__head"><h2>{chart.title}</h2><StatusBadge state="verified">Garmin</StatusBadge></div><MetricChart chart={chart} /></article>)}</section> : <EmptyState title={`No ${domain} snapshot yet`} description="Load the selected period to see these read-only Garmin signals." action={<button className="button button--primary" disabled={busy} onClick={() => void start('/dashboard/refresh', { period_days: period })}>Load {period}-day snapshot</button>} />}{domain === 'training' && data?.report?.activities.length ? <section className="panel section-gap"><div className="panel__head"><h2>Activities</h2><StatusBadge state="verified">{data.report.activities.length} records</StatusBadge></div><div className="activity-table table-scroll"><table><thead><tr><th>Activity</th><th>Start</th><th>Duration</th><th>Distance</th><th>Heart rate</th><th></th></tr></thead><tbody>{data.report.activities.map((item) => <tr key={String(item.id ?? item.measured_at)}><td><b>{String(item.name)}</b><small>{String(item.type)}</small></td><td>{new Date(String(item.measured_at)).toLocaleString()}</td><td>{Math.round(Number(item.duration_minutes))} min</td><td>{item.distance_km ? `${Number(item.distance_km).toFixed(1)} km` : '—'}</td><td>{item.average_hr ? `${Math.round(Number(item.average_hr))} bpm` : '—'}</td><td>{item.id && <a className="text-link" href={`https://connect.garmin.com/modern/activity/${item.id}`} target="_blank" rel="noreferrer">Garmin</a>}</td></tr>)}</tbody></table></div></section> : null}</>
}
