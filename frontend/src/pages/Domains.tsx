...
export function DomainPage({ domain }: { domain: keyof typeof configs }) {
  const config = configs[domain]; const { data } = useQuery({ queryKey: ['dashboard'], queryFn: () => api<Dashboard>('/dashboard') }); const { start, busy } = useJobs(); const charts = data?.report?.charts.charts.filter((item) => config.chartIds.includes(item.id as never)) ?? []
  return <><PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description} actions={<div className="segmented" aria-label="Period"><button>Today</button><button className="active">7 days</button><button>30 days</button></div>} />{charts.length ? <section className="chart-grid">{charts.map((chart) => <article className="panel" key={chart.id}><div className="panel__head"><h2>{chart.title}</h2><span className="source-chip">Garmin</span></div><MetricChart chart={chart} /></article>)}</section> : <EmptyState title={`No ${domain} snapshot yet`} description="Generate a weekly snapshot to load these read-only Garmin signals." action={<button className="button button--primary" disabled={busy} onClick={() => void start('/reports/weekly', {})}>Generate snapshot</button>} />}{domain === 'training' && data?.report?.activities.length ? <section className="panel section-gap"><div className="panel__head"><h2>Activities</h2><span>{data.report.activities.length} records</span></div><div className="activity-table table-scroll"><table><thead><tr><th>Activity</th><th>Start</th><th>Duration</th><th>Distance</th><th>Heart rate</th><th></th></tr></thead><tbody>{data.report.activities.map((item) => <tr key={String(item.id ?? item.measured_at)}><td><b>{String(item.name)}</b><small>{String(item.type)}</small></td><td>{new Date(String(item.measured_at)).toLocaleString()}</td><td>{Math.round(Number(item.duration_minutes))} min</td><td>{item.distance_km ? `${Number(item.distance_km).toFixed(1)} km` : '—'}</td><td>{item.average_hr ? `${Math.round(Number(item.average_hr))} bpm` : '—'}</td><td>{item.id && <a className="text-link" href={`https://connect.garmin.com/modern/activity/${item.id}`} target="_blank" rel="noreferrer">Garmin</a>}</td></tr>)}</tbody></table></div></section> : null}</>
}
