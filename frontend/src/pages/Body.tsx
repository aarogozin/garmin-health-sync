...
export function BodyPage() {
  const { data } = useQuery({ queryKey: ['dashboard'], queryFn: () => api<Dashboard>('/dashboard') }); const { jobs, start, busy, run } = useJobs(); const [historyJob, setHistoryJob] = useState<string | null>(null); const history = historyJob ? jobs[historyJob]?.result : null; const chart = data?.report?.charts.charts.find((item) => item.id === 'body-kg')
  return <><PageHeader eyebrow="RENPHO + Garmin" title="Body" description="Weight, composition and circumference measurements with their source preserved." actions={<button className="button" disabled={busy} onClick={async () => setHistoryJob(await start('/renpho/history'))}><Ruler />Load measurements</button>} />
    <section className="body-grid"><article className="panel body-hero"><span className="eyebrow">Latest measurement</span><div className="body-value"><Weight /><strong>{data?.latest_body ? `${data.latest_body.weight_kg.toFixed(1)} kg` : '—'}</strong></div><p>{data?.latest_body ? new Date(data.latest_body.measured_at).toLocaleString() : 'Connect RENPHO, then refresh to load body data.'}</p><div className="button-row"><button className="button" disabled={busy} onClick={() => void start('/renpho/latest')}><RefreshCw />Refresh</button>{data?.latest_body && <button className="button" onClick={() => void run(() => download('/reports/renpho/download', `renpho-body-composition-${data.latest_body!.measured_at.slice(0, 10)}.pdf`))}><Download />Report</button>}</div></article><article className="panel panel--wide"><div className="panel__head"><h2>Composition trend</h2><span className="source-chip">Combined sources</span></div>{chart ? <MetricChart chart={chart} /> : <EmptyState title="No body trend loaded" description="Generate a weekly snapshot to combine Garmin and RENPHO history." />}</article></section>
