...
export function BloodPressurePage() {
  const { data } = useQuery({ queryKey: ['dashboard'], queryFn: () => api<Dashboard>('/dashboard') }); const chart = data?.report?.charts.charts.find((item) => item.id === 'blood-pressure'); const { start, busy, run } = useJobs(); const [preview, setPreview] = useState<Values | null>(null)
  ...
  const submit = (values: Values) => run(async () => { await api('/pressure/preview', { method: 'POST', body: JSON.stringify(values) }); setPreview(values) })
